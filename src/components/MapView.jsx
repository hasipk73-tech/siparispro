...
function buildFullAddress(address, neighborhood) {
  return `${address}, ${neighborhood}, İstanbul, Türkiye`;
}
...
export default function MapView({ address, neighborhood }) {
    ...
    return (
      <div className="map-placeholder">
        ...
        <p className="map-placeholder__text">
          Harita için <code>.env.local</code> dosyasına
          <br />
          <strong>REACT_APP_GOOGLE_MAPS_KEY</strong> ekleyin
        </p>
        <a
          href={mapsLink}
          target="_blank"
  ...
  const staticUrl =
    ...
    `&size=480x220` +
    `&maptype=satellite` +
    `&markers=color:red%7Clabel:%7C${encoded}` +
    `&key=${MAPS_KEY}`;
  ...
  return (
    <div className="map-container">
      <img
        src={staticUrl}
        alt={`Uydu görüntüsü: ${fullAddress}`}
        className="map-image"
        loading="lazy"
      />
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="map-open-link map-open-link--overlay"
      >
